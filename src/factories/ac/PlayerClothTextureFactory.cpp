...
std::string playerClothArchivePath(uint32_t index) {
    ...
    const int length = std::snprintf(
        path, sizeof(path), "ac/texture/forest_1st/player/cloth-%03u.OTEX", index);
...
PlayerClothSpecification requirePlayerClothSpecification(uint32_t index) {
    if (index >= kPlayerClothCount) {
        throw std::runtime_error(
            "AC:PLAYER_CLOTH_TEXTURE cloth_index must be between 0 and 254");
    }
    return {
...
bool isPlayerClothArchivePath(const std::string& path) {
    ...
    if (path.size() != prefix.size() + 3U + suffix.size() ||
        path.compare(0, prefix.size(), prefix) != 0 ||
        path.compare(path.size() - suffix.size(), suffix.size(), suffix) != 0) {
        return false;
    }
    const size_t digit = prefix.size();
    if (path[digit] < '0' || path[digit] > '9' ||
        path[digit + 1] < '0' || path[digit + 1] > '9' ||
