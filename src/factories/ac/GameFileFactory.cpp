...
const Specification& RequireSpecification(YAML::Node& node) {
    ...
    if ((role == "dol" && storedSize != logicalSize) ||
        (role == "dvd" && storedSize != Align32(logicalSize))) {
    ...
    if (!range.IsMap() || range.size() != 3 ||
        GetSafeNode<uint64_t>(range, "source_offset") != found->sourceOffset ||
...
std::optional<std::shared_ptr<IParsedData>> GameFileFactory::parse(
...
ExportResult GameFileBinaryExporter::Export(std::ostream& write,
    ...
    if (data->archivePath != specification.archivePath ||
        data->bytes.size() != specification.storedSize) {
    ...
    write.write(reinterpret_cast<const char*>(data->bytes.data()),
                static_cast<std::streamsize>(data->bytes.size()));
    if (!write) throw std::runtime_error("AC:GAME_FILE raw export failed");
