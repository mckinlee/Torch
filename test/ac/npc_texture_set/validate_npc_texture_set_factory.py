...
def literal_yaz0(data: bytes) -> bytes:
    ...
    for start in range(0, len(data), 8):
        ...
        encoded.append((0xFF << (8 - len(chunk))) & 0xFF)
...
def main() -> int:
    ...
    compressed = literal_yaz0(bytes(rel))
    ...
    try:
        ...
        with zipfile.ZipFile(positive / "out" / "game.o2r") as archive:
            ...
            for variant, offset in enumerate(SET_OFFSETS, 1):
                validate_entry(
                    archive.read(f"ac/texture/npc/cat/cat-{variant:02d}.ANTX"),
                    bytes(rel[offset:offset + SET_SIZE]),
                )
        ...
        write_sparse(corrupt / "source.bin", [
